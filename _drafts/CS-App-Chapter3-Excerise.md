...
* 3.46
    * A. 在第三行执行完之后，首先将保存的局部变量%rbx压栈，在分配16个字节的栈空间。
    ...
    * C. 这个程序试图返回到地址0x040034，低位2字节被字符‘4’和结尾的null字符覆盖了。
    * D. 寄存器%rbx的保存值设置为0x3332313039383736。在`get_line`返回前，这个值会被加载会这个寄存器中。
    * E. 对malloc的调用应该以`strlen(buf)+1`作为它的参数，而且代码还应该检查返回值是为NULL。
